...
$courses_result = getStudentCourses($conn, $student_id);
$courses = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
            ...
            $schedule_id = isset($_POST['schedule_id']) && !empty($_POST['schedule_id']) ? intval($_POST['schedule_id']) : null;
                ...
                // Validate that the selected course (if any) is one the student is enrolled in
                if ($course_id !== null) {
                    $valid = false;
                    foreach ($courses as $c) {
                        if (intval($c['course_id']) === $course_id) { $valid = true; break; }
                    }
                    if (!$valid) {
                        $_SESSION['flash_message'] = "Invalid course selection. You can only link to your enrolled courses.";
            ...
            if (savePersonalScheduleSlot($conn, $student_id, $title, $schedule_type, $day_of_week, $start_time, $end_time, $location, $description, $course_id, $schedule_id)) {
                    ...
                    <td class="schedule-cell <?php echo !empty($slot_items) ? 'has-class' : ''; ?>">
                        <?php foreach ($slot_items as $item): ?>
                                <div class="schedule-item <?php echo getPersonalScheduleTypeClass($item['schedule_type']); ?>"
                                   ...
                                   <span class="schedule-course" title="<?php echo htmlspecialchars($item['course_name'] ?? $item['course_code']); ?>">(<?php echo htmlspecialchars($item['course_code']); ?>)</span>
                                   <?php endif; ?>
        ...
        <div class="schedule-list">
            ...
            <div class="schedule-list-item">
                <div class="schedule-list-info">
                    ...
                    <?php if (!empty($item['course_code'])): ?>
                    <span class="schedule-list-course">(<?php echo htmlspecialchars($item['course_code']); ?>)</span>
                    <?php endif; ?>
...
// Client-side validation: ensure selected course is from dropdown options
document.getElementById('scheduleForm').addEventListener('submit', function(e) {
    ...
    if (select) {
        ...
        if (val !== '') {
            ...
            if (!found) {
                e.preventDefault();
                alert('Invalid course selection. Please choose from your enrolled courses.');
                return false;
            }
        }
    }
